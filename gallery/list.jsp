...
<body>
	...
	<div class="container">
		...
		<div id="list">
		    ...
		    <% while(rs.next()){ %>
		    <!--rs.getString(콜롬이름) -->
			<div style="background-image:url(images/<%= rs.getString("filename") %>);" data-toggle="modal" data-target="#exampleModal" 
			 		                  onclick="showImage('<%= rs.getString("subject") %>', '<%= rs.getString("filename") %>', '2022-06-24 12:30:00');">
			 		 <span title="delete" onclick="location.href='delok.jsp?seq=<%= rs.getString("seq") %>';">&times;</span>
	...
	<script>
		...
		function showImage(title, img, regdate) {
			//alert(img);
